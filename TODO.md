...
#### Action activator, action effect, targeting spec, and action timing ToString for item and action details

Generic, hand-written-once formatting for `IActionActivator` (`PotionActivator`/`ScrollActivator`/`WandActivator`/`SpellActivator`/`DirectAction`), `ActionEffect`/`IActionEffectEntry` (`DirectDamage`/`DirectHeal`/`StatusEffectGrant`/`StatModifierGrant`/`AuraSourceGrant`/`HotkeyExpansionGrant`/`ChainedEffect`/...), `TargetingSpec`, and `ActionTiming` -- so `ItemDetailsWindow`'s Effects section (today a placeholder `entry.GetType().Name` per entry -- see the item details window entry above) and Activation section (today a hand-picked per-activator-type `switch` in `ItemDetailsWindow.BuildActivationSection`) both read consistently without hand-writing formatting per item. `ActionDefinition` shares `ActivatableDefinition` with `ItemDefinition`, so this is automatically reusable by a future Magic Menu (see that item below) too, not just items.

#### Item Details Comparison

Side-by-side comparison of two or more items' details, reusing the generic `ItemDefinition`-diff utility the item details window entry above originally called for (green/red/normal per-stat highlighting). Opened either from an inventory item's context menu ("Compare") followed by clicking another item, or from `ItemDetailsWindow`'s own title bar via a Compare button followed by clicking another item -- either path can keep clicking further items to add them to the same comparison; right-click stops adding. This is where that original entry's generic stat-diff-highlighting and equipped-item-comparison scope both end up now that the plain details-window half has landed on its own.
...
#### Child window placement rules

A general rule for where a child window (`ItemDetailsWindow`, `AbilityScoreWindow`, a corpse/loot window, ...) opens relative to its own parent: side-by-side on whichever side of the parent currently has the most room, rather than always to the right; and avoid overlapping a parent's-sibling window, then the parent itself, then a sibling window, in that priority order -- only actually overlapping one of those if no non-overlapping placement is possible. Every current child-window opener (`ItemDetailsWindowController.Open`, `SecondaryInventoryWindowController.OpenLoot`, `InventoryFolderController.CreateAbilityScoreWindow`) still uses the simpler "fixed gap to the right of the parent's own live position" idiom this item would replace.
