...
See `PLAN-body-parts.md` for the full implementation plan (data model, dispatching facades, execution phases). Health and health regen overhaul, halfway between Fallout's per-limb HP and Dwarf Fortress's full simulation. Every entity keeps a single current/maximum pool by default -- today's `HealthComponent`/`HealthRegenSystem`/`HealthDamage`/`HealthHeal` (`Game/Modules/Health/`), renamed `SimpleHealthComponent`/`SimpleHealthRegenSystem` once `ComplexHealth` exists alongside them, so the "plain" path keeps its own explicit name rather than silently becoming the assumed default with no qualifier. A second, opt-in path -- `ComplexHealth` -- gives complex entities (crawlers, bosses) a set of individually-tracked body parts instead of one pool. Whether an entity uses Simple or Complex health is decided entirely by which components its blueprint grants at Build time: a `SimpleHealthComponent` (`PackedComponentPool`, today's shape, unchanged) or one-or-more `BodyPartComponent` entries (`MultiComponentPool<BodyPartComponent>`, the "0..N components of the same type per entity" shape the Skills item above already anticipates for a similar per-entity/per-key store) -- no separate marker component needed to say "this entity is Complex," the same way `NonBlockingComponent.Kind` folds its own exemption-kind flag into the one component that grants the exemption rather than a second component that could drift out of sync (see CLAUDE.md's World & Map note). An entity is never expected to carry both.

**Race-defined body part templates.** Each race blueprint (`Game/Blueprints/Races/`) that wants `ComplexHealth` specifies its own body part list -- name/`BodyPartType` (see the BodyPartType followup below), a min-max HP range (mirroring how `MaximumHealth` is already a per-race `const` rolled against `mathUtility.Next` for starting `CurrentHealth`, e.g. `Goblin.MaximumHealth`/`Build`), and whether the part is Vital. At Build time, a `ComplexHealthEffects.GrantBodyParts`-shaped helper (name open) rolls each part's starting HP within its race-defined range and `Merge`s one `BodyPartComponent` per part onto the entity. A race that never calls this stays on `SimpleHealthComponent` by default -- Player/Goblin/Fairy/Ghost all keep Simple health at first landing; ComplexHealth is opt-in per race, not a blanket replacement.

Each race's body part list is fully independent, not a shared humanoid template with per-race overrides -- there's no assumption that every Complex race has the same set of parts, just different HP ranges on them. A Fairy would carry two Wing parts a Goblin's own list has no equivalent for; a cat-shaped race would carry four Leg/Foot-`BodyPartType` parts and zero Arm/Hand parts, rather than reusing a biped's arm slots for its front legs. This is also why `BodyPartType` (see that followup below) needs to be a real per-part field checked by type, not inferred from list position or count -- nothing about "part index 2" can be assumed to mean the same thing across two differently-shaped races.

**Total HP is a derived sum, not a stored field.** The health bar (`PlayerHealthBarContent`, `MapWindow.DrawHealthBar`, `HealthBarElement`) and anything else that wants "current/max HP" for an entity needs one shared query that works for both paths -- e.g. `HealthQueries.TryGetTotals(entityId) -> (current, maximum)`, checking `SimpleHealthComponent` first, else summing every `BodyPartComponent` the entity owns via the `MultiComponentPool`'s per-entity enumeration. Centralizing this in one place, rather than every consumer branching Simple-vs-Complex itself, is the same reasoning that already drove `IMapQuery.IsBlocking` to be the single chokepoint for the Blocking/NonBlocking decision.

**Regen ticks one body part at a time.** `SimpleHealthRegenSystem` keeps its current per-tick behavior unchanged (one Update call, one component, done). A new `ComplexHealthRegenSystem` instead picks, per due entity, the single body part with the lowest *current/maximum percentage* -- not lowest raw HP, since a 5/10 arm is more wounded than a 40/100 torso -- and applies that tick's regen amount to it alone, skipping any part still inside its post-disable lockout (see below). The regen math itself (Constitution-scaled `AbilityScoreMath.Lerp`, `StatModifierMath` for `HealthRegen`/`MaximumHealth` modifiers, `ProcessingTier`-scaled `secondsPerVisit`) is the same shape `SimpleHealthRegenSystem` already uses -- worth factoring the shared per-tick-amount computation out into one static helper both systems call, rather than copy-pasting `HealthRegenSystem.Update`'s math a second time.

**Vital parts and death.** A `SimpleHealthComponent` entity still dies when its single pool hits 0 (unchanged `HealthDamage.Apply`/`EntityDiedEvent` behavior). A `ComplexHealth` entity dies the instant *any* Vital body part hits 0, independent of the entity's summed total -- a torso and a head both Vital, arms/legs not, means losing either one kills the entity even while its total HP (arms/legs still intact) reads well above 0. This means the health bar's summed-total display and "is this entity dead" are no longer the same check for Complex entities the way they are for Simple ones today (`wasAlive = beforeHealth.CurrentHealth > 0` in `HealthDamage.Apply`) -- a `ComplexHealthDamage` counterpart's own death check needs to scan for `Vital && CurrentHealth == 0` on every hit to a Vital part, not just compare a sum against 0.

**Non-vital parts disable, not die.** A non-Vital part hitting 0 marks itself disabled rather than killing the entity -- future gameplay TODOs (see the BodyPartType followup below) will read this to gate movement/attacks/item pickup by which parts are currently disabled, but the disabled state itself (a bool, or `CurrentHealth == 0` read directly -- still open which) needs to exist now so those features have something to key off later.

**10-second post-disable regen lockout.** Once a part hits 0 and disables, `ComplexHealthRegenSystem` must not select it again until 10 real seconds have passed -- otherwise a part sitting right at the boundary between an incoming damage tick and a regen tick flickers disabled/enabled every other tick (the yo-yo effect between competing damage and healing). Needs a per-part expiry timestamp/countdown, most naturally the same `CountdownTicker`/`ITickCountdown` pattern already used for `PotionCooldownComponent`/`AuraSourceExpiryComponent` (`Game/Modules/StatusEffectAura/`) -- likely a field directly on `BodyPartComponent` (set on the hit that brings it to 0) rather than a separate sparse pool, since every disabled part needs exactly one such timer and `BodyPartComponent` already exists per part.

**Attacks hit a random body part (for now).** Until the targeting followup below lands, any damage source resolving against a `ComplexHealth` entity (melee, `DirectDamage`, contact damage, DoT ticks) picks one-or-more body parts at random to absorb the hit rather than any specific one -- `HealthDamage.Apply`'s current single-`HealthComponent` update needs a `ComplexHealthDamage` counterpart that, given a target entity, resolves which `BodyPartComponent`(s) take the amount (`mathUtility.Next` over the entity's own part list) before applying the same `IncomingDamage`/`MaximumHealth` modifier chain per part that `HealthDamage.Apply` already does for the whole pool. Whether "one or multiple" per hit is itself randomized, fixed per attack, or always exactly one until the targeting followup adds real multi-part hits (lava hitting legs, a fireball hitting everything) is an open call -- start with exactly one part per hit as the simplest version, since the multi-part case is explicitly the next followup's job, not this one's.

**Open design questions, deliberately not decided here:** whether `BodyPartComponent`'s disabled/Vital/lockout fields live on the component itself or split across a couple of sparse companion pools (mirroring `ForceBlockingComponent`/`NonBlockingComponent`'s own precedent for small, focused marker pools versus one component carrying everything); whether `HealthDamage`/`HealthHeal` become Simple-vs-Complex-dispatching facades over two concrete implementations, or whether callers (melee, `ActionEffectResolver`, `ContactDamageSystem`, DoT tick systems) branch themselves; and exactly how a race blueprint's body-part template data is authored (a `record`/array literal per race, similar to how `PlayerBlueprint`/`Goblin`/etc. already hardcode `MaximumHealth` as a `const`, versus a more data-driven table like `SpriteManifest.json`).

Several followups, each blocked on this landing first, are logged as their own TODO items below (Game and Presentation both) -- see `PLAN-body-parts.md`'s own "Not in scope" section for the full, up-to-date list rather than duplicating an enumeration here that would only drift out of sync as more get added.

#### Targeted body part damage and multi-part effects

Blocked on the Body parts item above landing with its "random part" placeholder. Real per-part targeting for damage/effects, replacing that random pick: a single-target attack should be able to land on one specific part (once a targeting UI exists to choose it -- not designed here), and an area/environmental effect should be able to resolve against multiple parts at once by its own rule rather than uniformly-random -- e.g. lava contact damage (`Game/Modules/Burning/`, `ContactDamageSystem`) hitting legs specifically (a creature standing in lava is burning its feet, not its head), while a Fireball's `Burst` applies its damage/Burning equally across every part instead of picking one winner. Needs `ActionEffect`/`IActionEffectEntry` (`Game/Modules/Actions/Effects/`) to carry an optional body-part-selection rule per entry -- a new small enum or delegate (`Random` today's default, `Lowest`/`AllParts`/a specific `BodyPartType` for the lava/fireball cases) that `ComplexHealthDamage`/whatever applies status effects to a part reads instead of always rolling random. `ActionEffectResolver.Apply`'s existing caster-then-target modifier chain (see the ActionEffectResolver damage/heal consistency item above) would need to run once per selected part rather than once per hit.

#### BodyPartType categorization and gameplay effects

Blocked on the Body parts item above landing. Each `BodyPartComponent` needs a `BodyPartType` (Head/Torso/Arm/Leg/Hand/Foot/... -- exact set not decided here) so other systems can key off *kind* of part rather than by name string. Two concrete consumers already anticipated: Equipment (see this TODO's own updated Equipment entry above -- slot count/availability keyed by which typed parts an entity currently has, e.g. a ring per finger); and actions/movement gated by a part's own disabled state -- Legs disabled slows or blocks movement (`MovementSystem`), Arms disabled blocks melee/lifting (`ActionEffectResolver`'s Adjacent-targeted attacks, `InventoryActions` pickup), the concrete "many future features" the original Body parts item flags above. No system reads a disabled part's state at all until this item lands -- the state exists (see the Body parts item's own "non-vital parts disable" note), nothing consumes it yet; this item is that consumption pass, once there's more than one gameplay system ready to key off it at once rather than wiring each in piecemeal.

#### Per-body-part vs whole-entity status effects

Blocked on the Body parts item above landing. `StatusEffectStack`/`StatusEffectAuraApplierRegistry` (`Game/Modules/StatusEffects/`) today apply every status effect at the entity level -- correct for something like Poison (a systemic effect, no reason to localize it to one part), wrong for something like Burning once ComplexHealth exists (a burning leg reads more naturally than a burning entity, and ties into the targeted-damage followup above -- lava burning the legs specifically should also apply Burning to the legs specifically, not the whole entity). Needs a way for a `StatusEffectGrant`/`IStatusEffectAuraApplier` to declare whether it's entity-scoped (today's behavior, unchanged default) or part-scoped, and for the part-scoped case, a place to actually track "this body part has N stacks of Burning" -- likely a second `MultiComponentPool`-shaped store keyed by (entityId, bodyPartId) rather than entityId alone, distinct from today's per-entity `StatusEffectStack`. Feeds directly into the HealthWindow item under Presentation, which needs to display per-part status effects once they can exist.

#### Limb-specific gameplay penalties beyond disable

Blocked on the Body parts item above and its BodyPartType categorization follow-up both landing
first -- this extends whatever binary disabled-at-0 gameplay hooks that follow-up wires (Legs
