...
#### Checkbox widget to replace the Hide Disabled toggle button

The Tab Stats row's Hide Disabled control (see the Tab Stats row item above) ships as a click-to-toggle button first (Outset/Inset, the same convention `TabbedContent`'s tab tiles already use for selected/unselected) -- a real checkbox reads more standard for a binary on/off filter than a button that happens to change border style. Blocked on the Standard widget set item below, which doesn't include a checkbox yet.
