...
**Follow-up worth designing next, not yet built:** reuse this exact same search-box-with-ghost-text control (not just the `TextBox.GhostText` primitive, the whole debounced-filter pattern) for two more places: the item sorting/filtering/searching item above (filtering items within a tab, not just which tabs are visible), and the Tab Stats row item below (both its own search box and, less obviously, its item-count display -- a live count of *currently filtered/visible* items, not just the tab's total, once item-level filtering exists alongside tab-level filtering). Worth deciding whether the debounce/filter logic itself should get pulled out of `TabbedContent` into something shared before a second consumer copies it by hand.

#### Tab Stats row

A row of data between the tab strip and the inventory grid, scoped to whichever tab is currently active: number of items, total weight (blocked on the Item weight and carry capacity scaling with Strength item below -- no `ItemDefinition`/`InventoryItemStackComponent` carries a weight field yet), plus the sort/filter/search controls from the Inventory item sorting, filtering, and searching item above. See that item's own follow-up note on reusing the Tab search box/ghost-text/debounce pattern here too.
