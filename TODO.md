...
**Known limitation, deliberately not fixed here:** every added column repositions *all* open columns (closed and rebuilt fresh each time, chained off the anchor's live position), and a third-or-later column can spawn off-screen with nothing to pull it back on-screen -- confirmed during testing. This is exactly the still-open "Child window placement rules" item below's own scope (shrink/wrap/avoid-overlap positioning for child windows generally), not solved specifically for this feature.
