...
#### Per-pool entity capacity for rare component types (landed)

`ComponentManager.RegisterPackedPool`/`RegisterMultiPool` (`Engine/ECS/Components/ComponentManager.cs`) now take optional `maximumEntityCount`/`initialCapacity` overrides instead of always reading the manager's own world-scale fields (`GameLoop.InitialEntityCapacity = 2_600_000`). A naive smaller seed alone wasn't safe on its own, though: entity ids are assigned globally and densely across the whole world, not per-component-type, so any entity could in principle need any component -- `PackedComponentPool<T>`/`MultiComponentPool<T>` now bounds-check every entity-indexed accessor (mirroring `DirectComponentPool`'s pre-existing `IsInBounds` pattern) and grow themselves on demand (reusing the same doubling formula `EntityManager.CreateEntity` already used, now shared via `Engine/ECS/Components/EntityCapacityGrowth.cs`) the moment an entity id beyond the seed is actually used. `ComponentManager.ResizeEntityCapacity` skips any pool registered with a `maximumEntityCount` override, so it isn't forced back up to world scale the next time an unrelated entity creation grows the global capacity -- an `initialCapacity`-only override (dense/instance-count storage) doesn't need this, since `ResizeEntityCapacity` never touches the dense side to begin with.

`RegisterDirectPool` deliberately did **not** gain this: every one of its registrants (`TransformComponent`, `SpriteComponent`, etc.) is genuinely near-universal -- the entire reason Direct pool exists -- so the override would add branch overhead to the engine's hottest pool for no real type to ever use it. The one Direct-registered type that actually was rare, `InventoryDisabledComponent`, was reclassified to Packed instead (matching every other rare component here), which also gets it a dense-side reduction Direct pool structurally can't offer.

Applied to `AchievementUnlockedComponent`/`ForceBlockingComponent`/`NonBlockingComponent`/`ActionHotkeyBindingComponent`/`HotkeyExpansionUnlockComponent`/`ScrollMasteryComponent`/`CrawlerComponent`/`CorpseLootedComponent`/`InventoryDisabledComponent`/`ItemHotkeyBindingComponent`; `ClassComponent`/`RaceComponent`/`ExpiringStatModifierComponent` deliberately left unbounded (all three grow rapidly with entity generation). Live diagnostics measurement (`Log/diagnostics/latest.json`'s `Memory` section, fresh runs before/after): **1,174.24MB -> 1,031.51MB total, a 142.73MB (12.2%) reduction** across the 42 registered pool types. Components with zero live instances this session (`ForceBlockingComponent`, `ScrollMasteryComponent`, `InventoryDisabledComponent`) shrank to a few hundred bytes each, down from ~35-40MB apiece.

The Player-only components (`AchievementUnlockedComponent`/`ActionHotkeyBindingComponent`/`ItemHotkeyBindingComponent`/`HotkeyExpansionUnlockComponent`) initially landed a smaller-than-hoped win (e.g. `ActionHotkeyBindingComponent` 39.12MB -> 25.47MB, not near-zero), because `FloorBuilder.CreatePlayer` ran *after* `PopulateFloor`, giving the Player a high entity id (~2.1M in that measurement) that these entity-id-indexed arrays still had to grow to cover. Fixed by reserving the Player's entity id up front: `FloorBuilder.ReservePlayerEntity` (new, one line -- `ecsContext.EntityManager.CreateEntity()`) is now called from `WorldSessionBootstrapper.Build` immediately once `ecsContext` exists, *before* `PopulateFloor` runs -- `FreeIdPool.Rent()`'s first call against a fresh pool deterministically returns 0, so the Player now always lands on entity id 0. `CreatePlayer` itself still runs at the same point as before (after `PopulateFloor`, since its free-cell placement search needs the map already populated) but now takes the reserved id as a parameter instead of minting its own. Confirmed safe: no consumer anywhere assumed entity ids 0..N are contiguously populated (every pool already gates on its own presence-tracking), `IPlayerQuery`/`World.PlayerEntityId` is unaffected (still only set once `CreatePlayer` completes, preserving the existing "-1 until spawned" window `HotbarContent` depends on), and `PlayerBlueprint.Build`/`ComponentManager.Merge` never cared *when* an id was minted, only what's currently merged onto it.

Re-measured after the fix: the four Player-only pools dropped from ~0.7-25MB each to under 1KB (`AchievementUnlockedComponent` 25,466,648 -> 728 bytes; `ActionHotkeyBindingComponent`/`ItemHotkeyBindingComponent` 25,466,832 -> 888 bytes each; `HotkeyExpansionUnlockComponent` 8,488,692 -> 44 bytes), a further 80.95MB saved on top of the earlier pass -- **1,174.24MB -> 950.56MB total, a 223.68MB (19.1%) reduction** from the original baseline across all 42 registered pool types.

`CrawlerComponent`/`CorpseLootedComponent` only got a dense-capacity reduction (220,000 default down to a few thousand/dozens) -- `maximumEntityCount` was deliberately left at the world-scale default for both, since either can land on any entity in the population (`TestMapBuilder.CrawlerPercent` rolls a Crawler on any newly-generated NPC; any corpse can be looted), so bounding the entity-index side would just mean constant regrowth with no real saving, unlike the Player-only cases above.
...
#### Component ToString coverage for the Admin inspector

InspectionWindowContent's Admin-mode component dump (`Presentation/UI/Content/InspectionWindowContent.cs`, via `ComponentInspector` -- the Inspection V2 successor to the retired `SelectionWindowContent`) displays whatever `ToString()` a followed entity's components return -- most component structs still fall back to the default `ToString()` (the type name only, no field values), so the dump shows little beyond "this entity has a HealthComponent" without the actual numbers. `HealthComponent` is the one existing example of a component with a real, informative `ToString()` (a percentage bar plus current/max, see its own doc comment on why it degrades gracefully for an invalid MaximumHealth rather than throwing). Worth a pass giving every component struct (or at least the ones a player/dev would actually want to inspect -- `ManaComponent`, `AbilityScoreComponent`, `StatModifierComponent`, `InventoryItemStackComponent`, etc.) an equivalent field-dump `ToString()`, so the dump actually earns its name instead of just confirming presence/absence.
...
Consolidates the old "Item inspection popup"/"Item details window" entries (click an inventory cell, see its full name/description/tags/properties) into something more thorough: a persistent details pane that shows whatever item is currently *selected* (not just a click-to-open popup that closes again) -- name, description, tags, and every stat, mirroring how `InspectionWindowContent`'s Basic mode already shows whatever *entity* is currently selected on the map.
