...
#### Entity striping responsibility split between Engine and Game (audited, landed)

Audited: confirmed clean. `Engine.ECS.Systems.EntityStripeSet`/`TieredEntityStripeSet` are still genuinely tier-ignorant -- grepping all of `Engine/` for `ProcessingTierLevel`/`ProcessingTierComponent`/`ProcessingTierDivisors` returns nothing. No Game-layer system has duplicated `EntityStripeSet`'s bucket-membership math either -- grepping `Game/` for hand-rolled `entityId % stripeCount`-shaped logic returns nothing outside Engine itself. All twelve `TieredEntityStripeSet` consumers (Movement, StatusEffectAura x2, StatModifierExpiry, Burning, ContactDamage, ActionLock, ManaRegen, HealthRegen, ActionCooldown, ProcessingTierSystem itself) consistently go through `Game.Modules.ProcessingTier.ProcessingTierWiring.CreateAndWire` rather than hand-wiring a `TieredEntityStripeSet`.

The one asymmetry flagged in the original version of this item was real: `ProcessingTierWiring.CreateAndWire` centralized the tiered construct+subscribe dance, but the plain non-tiered `EntityStripeSet` case had no equivalent -- `DelayedActionSystem`/`ActionActivationSystem`/`TestCombatBehaviorSystem`/`ConsumableActivationSystem` each hand-rolled the same three-line `new EntityStripeSet(...)` + `EntityAdded +=` + `EntityRemoved +=` wiring. Closed by adding `EntityStripeSet.CreateAndWire(stripeCount, IEntityMembershipPool)` (`Engine/ECS/Systems/EntityStripeSet.cs`) -- lives in Engine itself, unlike `ProcessingTierWiring`, since this wiring needs no game-specific knowledge. All four call sites now use it.
