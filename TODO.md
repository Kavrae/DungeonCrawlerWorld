...
#### Burning/Poison stack representation -- investigate a single updating magnitude instead of N discrete stack components

Today a stack of Burning or Poison is represented twice: once as an integer (`BurningTimerComponent.StackCount`/`BodyPartBurningTimerComponent.StackCount`/`PoisonTimerComponent.StackCount`) and once as N separate, structurally-interchangeable `StatusEffectStack`/`BodyPartStatusEffectStack` instances in a `MultiComponentPool` (one instance added per `ApplyStack` call, one removed per tick) -- see `Game/Modules/StatusEffects/StatusEffectStack.cs`, `BurningEffects.ApplyStack`, `BurningAuraApplier.ApplyBodyPartScopedStack`, and `BodyPartBurningSystem.Tick`'s own "find one matching stack instance to remove" linear search. Worth investigating collapsing this to a single updating magnitude value with no separate N-instance pool at all, which would remove the per-tick `MultiCountdownTicker`-shaped equality-matched removal dance for that pool and simplify `HealthWindow`'s remaining-duration formula (`FramesUntilNextTick + (StackCount-1)*TickIntervalFrames` -- see `HealthWindow.TryGetBodyPartBurningLine`) down to a plain countdown.

Not a pure simplification, though -- the two mechanisms produce genuinely different decay behavior, not just different code for the same behavior. Today's "N discrete stacks, exactly one decays per tick regardless of new grants" gives graceful decay (5 stacks -> 5 damage -> 4 -> 3 -> ...) as a side effect of the representation alone. "One magnitude that resets its duration on re-application" needs that decay curve decided explicitly instead: either damage stays constant until a hard cutoff (simpler, but a more abrupt extinguish -- no lingering burn-out) or the magnitude needs its own decay-as-duration-nears-zero rule to reproduce today's taper, which is arguably *more* logic than a bare per-tick decrement. Investigate which curve is actually wanted before committing to the representation change.

Also check the blast radius before starting: Poison (`Game/Modules/Poison/`) and Paralysis (`Game/Modules/Paralysis/`) both register through the shared `TimerBasedAuraApplier<T>` (`Game/Modules/StatusEffects/TimerBasedAuraApplier.cs`), which assumes `IStatusEffectAuraApplier.ApplyStack` means "add exactly one atomic unit" -- `StatusEffectAuraSystem.GrantStacks` tops off toward a target by calling it in a loop. A magnitude field can satisfy that same contract unchanged (`ApplyStack` becomes "bump magnitude by one and refresh duration," called the same N times) without touching Poison/Paralysis at all. Only widen the investigation to the shared `IStatusEffectAuraApplier` interface itself if the goal is a true O(1) "set to target" call instead of the loop -- that would ripple to every effect using `TimerBasedAuraApplier<T>`, not just Burning's own bespoke `BurningAuraApplier`.
