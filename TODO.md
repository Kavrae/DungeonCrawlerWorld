...
#### Entity striping responsibility split between Engine and Game

`Engine.ECS.Systems.EntityStripeSet` and `Engine.ECS.Systems.TieredEntityStripeSet` both live in Engine, deliberately kept ignorant of what a "tier" actually means -- `TieredEntityStripeSet`'s own doc comment is explicit that it never references a concrete tier concept (e.g. distance-from-player), taking a `currentTierIndexLookup` delegate instead. `Game.Modules.ProcessingTier.ProcessingTierWiring.CreateAndWire` is where the real `ProcessingTierComponent`/`ProcessingTierLevel` binding happens, specifically because (per its own doc comment) `TieredEntityStripeSet` "deliberately never does" reference those types. On paper this is a clean boundary -- worth actually auditing rather than assuming, though: confirm no Game-layer system has quietly duplicated `EntityStripeSet`'s bucket-membership logic instead of composing it, and confirm nothing Engine-side has drifted toward assuming ProcessingTier's specific tier count/shape.

One concrete asymmetry already visible: `ProcessingTierWiring.CreateAndWire` exists specifically to centralize the "construct a `TieredEntityStripeSet`, subscribe to the driving pool's `EntityAdded`/`EntityRemoved`, subscribe to `TierChanged`" dance that every tiered-striping system's constructor would otherwise repeat by hand (see its own doc comment). No equivalent exists for the plain, non-tiered `EntityStripeSet` case -- every system striping without tiers (e.g. `DelayedActionSystem`) still hand-rolls the same three-line `new EntityStripeSet(...)` + `EntityAdded +=` + `EntityRemoved +=` wiring in its own constructor. Since that wiring needs no game-specific knowledge at all (unlike the tiered case's `ProcessingTierComponent` dependency), it's a legitimate candidate for its own small Engine-layer helper (or an `EntityStripeSet` factory method) -- worth deciding whether that inconsistency is worth closing, or whether the boilerplate is small enough to leave alone.

#### AOE marker/effect splatting responsibility split between Engine and Game

Three separate Engine-layer primitives now cover overlapping pieces of "which cells does this AOE-shaped thing reach": `Engine.Math.DistanceFalloff` (the halves-per-tile falloff math plus `ScatterManhattan`'s cell-visiting loop), `Engine.Math.TargetShapeResolver` (live ability-targeting preview shapes), and `Engine.ECS.Systems.SourceSplatting` (the generic chain-walk-and-splat mechanic: iterate a `MultiComponentPool<TSource>` entity's own source instances, splat/unsplat via caller-supplied delegates). Each was pulled out of duplicated call sites independently and each has its own carefully-reasoned "no game-specific knowledge" doc comment (`SourceSplatting`'s is explicit that it "deliberately doesn't touch event subscription or position lookup" and "deliberately doesn't own any ProcessingTier-based deferral" -- see its own doc comment for the full reasoning), but nothing has audited whether the three actually form a coherent set, or whether e.g. `TargetShapeResolver` quietly re-derives falloff-shaped logic `DistanceFalloff` already owns.

The "markers and effects" split is the more concrete half worth investigating: `Game.Modules.StatusEffectAura.AuraGrid`/`StatusEffectAuraSystem` (the gameplay per-effect-type stack accumulator) and `Presentation.UI.MapTintGrid` (the cosmetic, blended-RGB-per-cell visual overlay) are two fully independent per-cell structures, each separately subscribing to the same `AuraSourceAddedEvent`/`AuraSourceRemovedEvent`/`EntityMovedEvent` stream and each separately chain-walking the same source pool via `SourceSplatting.ScatterAll`/`ResyncEntity` on every add/move/remove -- `MapTintGrid`'s own doc comment already defends this as intentional (accumulation math that "has nothing in common between the two," plus no matching need for `StatusEffectAuraSystem`'s ProcessingTier-gated deferral since this is "a purely cosmetic overlay"), but that's a design decision made once, not something re-checked since. Worth confirming it still holds -- and whether the cosmetic grid genuinely needs to be a fully independent structure re-walking the same lifecycle a second time, or whether it could instead derive from data the gameplay grid already computed, now that `SourceSplatting` exists as shared machinery between them.
...
#### Investigate collapsing the Blocking/non-Blocking occupancy split into a unified, flag-filtered store

**Next item to start once the current `World.cs`/`Map.cs` cleanup and refactor pass wraps up.**

`Map` (`Game/World/Map.cs`) keeps three independent stores: `_blockingEntityIds` (one exclusive occupant per cell, flat array), `_nonBlockingEntityIdsByPosition` (any number of occupants per cell, sparse `Dictionary<int, List<int>>`), and `_terrainEntityIds` (a third, unrelated store). `World` (`Game/World/World.cs`) mirrors that split at every call site that touches occupancy -- `MoveEntity`/`MoveEntityUnchecked`/`PlaceEntityOnMap`/`RemoveEntityFromMap`/`ConvertToNonBlocking` all branch on `IsBlocking(entityId)` and call one of a parallel pair of footprint helpers (`AddBlockingFootprint`/`RemoveBlockingFootprint` vs. `AddNonBlockingFootprint`/`RemoveNonBlockingFootprint`). This is now a large fraction of both files' actual logic, and the two most recent cleanup passes over them were largely about keeping the two branches consistent with each other rather than eliminating the duplication itself.

Most tile/grid game architectures don't split storage by category at all: one multi-occupant store per cell (the shape `_nonBlockingEntityIdsByPosition` already has), with each entity tagged by a collision flag/bitmask (Solid, Flying, Phasing, etc.) and "is this cell blocked" resolved by filtering that cell's occupant list at query time instead of maintaining a second, exclusive array. Unity's `Physics2D` layer-collision-matrix and roguelike tile-flag systems (Dwarf Fortress, Cogmind) both work this way, and it generalizes cleanly if occupancy ever needs more than two categories (today's binary Blocking/non-Blocking split can't express, say, "blocks Ground creatures but not Flying ones" without another parallel branch).

The tradeoff to weigh, not assume an answer to: `_blockingEntityIds`' exclusive array gives an O(1) direct read for the common "is this cell blocked" check; a unified list-plus-filter trades that for a small linear scan per check (cheap given occupancy is low-density today, but not free, and not yet profiled). Worth a real investigation -- including whether `IsFootprintFreeFor`/`IsValidDestination` (`World.cs`) still make sense once occupancy is queried through one path -- before committing to a migration.
...
#### Player selection menu (non-debug)

A player-facing replacement for the current selection window -- `SelectionWindowContent`/`ComponentInspector` (`Presentation/UI/Content/SelectionWindowContent.cs`) is explicitly a debug/dev inspector today, dumping raw `ToString()` output for whatever components a selected entity happens to have (see the "Component ToString coverage for the selection inspector" item above, which is about improving that same debug view, not this one). This is a parallel, player-facing view instead: same "click an entity to select it" input path, but curated, human-readable content instead of a raw component dump, and gated by the observer's own Skill levels (see the Skills item under Game above) -- e.g. a low-level Perception-equivalent skill shows only name/rough health, a higher level reveals exact stats, resistances, or status effects. Needs Skills to actually exist first for the gating to mean anything.
...
#### Outlined glyph text for terrain-color visibility

`GlyphRenderer.Draw`/`DrawCentered`/`DrawRightAligned` (`Presentation/Rendering/GlyphRenderer.cs`) draw straight through `SpriteBatch.DrawString` with a single fill color -- a glyph can disappear against a similarly-colored terrain tile underneath it (e.g. a green-tinted entity glyph over Grass). Add an outline -- the common approach is drawing the same string a handful of times at a 1px offset in each direction in an outline color before the real fill pass -- so glyph text stays legible regardless of what's rendered beneath it. Affects every glyph draw call, not just entities: map node badges, HUD text, anything still glyph-rendered rather than sprite-rendered post-migration.
