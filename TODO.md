...
#### Scrolls (ActionEffect/ActionActivator foundation landed -- Scroll/Wand activators themselves still open)

`Game/Modules/Actions/` now exists: **`ActionEffect`** -- a composable `IReadOnlyList<IActionEffectEntry>`, each entry (`DamageEffectEntry`, `HealEffectEntry`, `ManaRestoreEffectEntry`, `HotkeySlotGrantEntry`, `StatusEffectGrantEntry`, `StatModifierGrantEntry`, `ChainedEffectEntry`, `AuraSourceToggleEntry`) owning its own application logic -- and **`IActionActivator`** -- `Guid Id`/`Targeting`/`Timing`/`IReadOnlyList<ActionEffect> Effects`/display fields, how/how-often an action triggers. `AbilityDefinition` (a spell) and `Game.Modules.Inventory.PotionActivator` both implement it today, both wrapping the same shared `ActionEffect`/`ActionTiming` shapes -- replacing the old separate `AbilityEffect`/`ConsumableEffect` types (see `PLAN-action-effect-activator.md` for the full design, including why per-activator-kind cost/consumption logic -- mana-spend, `PotionCooldown` abuse -- deliberately stayed in each system rather than becoming composable entries).

What's still open: a scroll is a one-time-use item that triggers an ability-like effect without a caster needing mana or the ability itself, the same way a potion delivers a fixed effect today -- landing it now means adding a `ScrollActivator : IActionActivator` (one-time-use, its own special targeting rules) alongside the existing `PotionActivator`, no further `ActionEffect` restructuring needed. A wand activator (see the Wands item below) would similarly spend a charge instead of mana. Also still needed: the item "use" action design the Consumable items entry above already flags as a prerequisite.
