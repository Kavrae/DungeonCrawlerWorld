...
/// the player can only drink one potion per cooldown regardless of which potion type. Does not
/// gate a second potion; it only decides whether ConsumableActivationSystem also grants a
/// punishment Poison stack (see ComputeAbusePoisonDurationTicks) alongside that second potion's
/// effect. The cooldown itself scales with the consumer's own Constitution (ComputeDurationFrames)
/// -- DurationFrames/MinDurationFrames are just its two endpoints, plus DurationFrames' own
/// fallback role when no Constitution score is available at all.
/// </summary>
