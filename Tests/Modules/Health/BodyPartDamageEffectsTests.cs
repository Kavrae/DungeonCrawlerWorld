...
public sealed class BodyPartDamageEffectsTests
{
    ...
    public void ApplyToPart_HitLandsAtZero_DisablesAndSetsFreshLockout()
    {
        ...
        BodyPartDamageEffects.ApplyToPart(bodyParts, denseIndex, statModifiers: null, entityId: 0, amount: 10, now: 0);
    ...
    public void ApplyToPart_SecondHitAgainstAlreadyZeroPart_RearmsLockoutFromFresh()
    {
        ...
        bodyParts.UpdateByDenseIndex(denseIndex, static (ref BodyPartComponent part) => part.RegenLockedUntilFrame = 5);

        BodyPartDamageEffects.ApplyToPart(bodyParts, denseIndex, statModifiers: null, entityId: 0, amount: 1, now: 0);

        var part = bodyParts.GetReadonlyByDenseIndex(denseIndex);
        Assert.AreEqual(0f, part.CurrentHealth);
        Assert.IsTrue(part.IsDisabled);
        Assert.AreEqual((uint)(10 * GameTiming.FramesPerSecond), part.RegenLockedUntilFrame, "The second 0-landing hit must reset the lockout to a fresh 10 seconds, not leave the partway-decayed value from the first hit.");
    }
    ...
    public void ApplyToPart_HitDoesNotReachZero_DoesNotDisableOrSetLockout()
    {
        ...
        Assert.AreEqual((ushort)0, part.RegenLockedUntilFrame);
    }
    ...
    public void ApplyToPart_ClampsAtZero_DoesNotGoNegative()
    {
        ...
        BodyPartDamageEffects.ApplyToPart(bodyParts, denseIndex, statModifiers: null, entityId: 0, amount: 100, now: 0);
